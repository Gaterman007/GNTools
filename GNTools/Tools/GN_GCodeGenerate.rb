...
require 'json'
require 'fileutils'
require 'GNTools/Tools/GN_DrillBits.rb'
...
module GNTools
  ...
  class GCodeGenerate
    ...
    def self.generate_header_comment
	  ";\n; === #{GNTools.traduire('G-Code généré par GNTools')} version: #{PLUGIN[:version]} ===\n;\n"
    end
    ...
    def self.generate_file_header(filename)
      ...
      header += "; #{GNTools.traduire('Par')}: GNTools v#{PLUGIN_VERSION[:version]}\n"
      header += ";"
      header
    end
