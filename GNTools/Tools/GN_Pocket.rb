	...
	module Paths
		...
		class Pocket < PathObj
			...
			class PocketData
				...
				attr_accessor :drillBitRayon
				attr_accessor :safeHeight
				attr_accessor :material_thickness
				attr_accessor :down_slow
				attr_accessor :height_pass
				attr_accessor :down_slow
				attr_accessor :defaultFeedRate
				attr_accessor :pocketsize
				attr_accessor :holeBottom
					
				def getPocketData(pocketObj)
					@loopFace = LoopFace.new()
					@loopFace.pfaces_vertices = pocketObj.getGlobal()
					@loopFace.pfaces_normal = pocketObj.pocket_faces_normal
					@drillbitSize = DrillBits.getDrillBit(pocketObj.drillBitName).cut_Diameter			# diametre de la drill
					...
					if pocketObj.multipass
						...
						@height_pass = @material_thickness - pocketObj.depth
					end
					if @safeHeight == nil
					...
					boundingbox = pocketObj.pathEntitie.bounds
					@holeBottom = @material_thickness - pocketObj.depth
				...
				def to_str
				  string =
					"Loops : #{loopFace}\n" \
					"Drill bits : #{drillbitSize} : #{drillBitRayon}\n" \
				...
				while @pd.height_pass >= @pd.holeBottom
					...
					if methodType == "Pocket"
						...
						while offset_loop > @pd.drillBitRayon.mm
							loopsleft = @pd.loopFace.deplacer(-offset_loop)
							self.createPathLoop(pathGroup,loopsleft,@pd.height_pass)
							offset_loop -= @pd.drillBitRayon.mm
						end
						loopsleft = @pd.loopFace.deplacer(-@pd.drillBitRayon.mm)
						self.createPathLoop(pathGroup,loopsleft,@pd.height_pass)
						
					elsif methodType == "Inside"
						loopsleft = @pd.loopFace.deplacer(-@pd.drillBitRayon.mm)
						self.createPathLoop(pathGroup,@pd.loopsleft,@pd.height_pass)
					elsif methodType == "Outside"
						offset_loop = @pd.pocketsize.mm
						loopsleft = @pd.loopFace.deplacer(@pd.drillBitRayon.mm)
						self.createPathLoop(pathGroup,loopsleft,@pd.height_pass)
					end
					@pd.height_pass -= @pd.down_slow
				end
			end
			...
			def createGCode(gCodeStr)
				...
				while @pd.height_pass >= @pd.holeBottom
					...
					if methodType == "Pocket"
						...
						while offset_loop > @pd.drillBitRayon.mm
							loopsleft = @pd.loopFace.deplacer(-offset_loop)
							gCodeStr = self.createGCodeLoop(loopsleft,@pd.height_pass,gCodeStr)
							offset_loop -= @pd.drillBitRayon.mm
						end
						loopsleft = @pd.loopFace.deplacer(-@pd.drillBitRayon.mm)
						gCodeStr = self.createGCodeLoop(loopsleft,@pd.height_pass,gCodeStr)
